...
$conteudo = @"
oh-my-posh init pwsh --config 'https://raw.githubusercontent.com/JanDeDobbeleer/oh-my-posh/refs/heads/main/themes/clean-detailed.omp.json' | Invoke-Expression
...
# WSL2
$opcao = Read-Host "Deseja instalar o WSL2 com Ubuntu 26.04? (Y)es / (N)o"

if ($opcao -match '^(Y|y)$') {
    Write-Host "`nIniciando instalação do WSL2 e Ubuntu 26.04..." -ForegroundColor Green
...
Write-Host "`n✅    GH Auth Login" -ForegroundColor Green
gh auth login

Write-Host "`n✅    AZ Login" -ForegroundColor Green
az login
